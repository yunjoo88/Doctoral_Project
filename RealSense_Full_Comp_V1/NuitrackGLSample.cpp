...
void NuitrackGLSample::onSkeletonUpdate(SkeletonData::Ptr userSkeletons)
{
	...
	//std::cout << "Number of Bodies : " << skeletons.size() << std::endl;
	NumOfBodies = skeletons.size();

	for (auto skeleton : skeletons)
	{
		drawSkeleton(skeleton.joints);
		HeadPos[0] = skeleton.joints[JOINT_HEAD].real.x;
		HeadPos[1] = skeleton.joints[JOINT_HEAD].real.y;
		HeadPos[2] = skeleton.joints[JOINT_HEAD].real.z;
		//std::cout << "Head Position : " << HeadPos[0] << ", "
		//								<< HeadPos[1] << ", "
		//								<< HeadPos[2] << std::endl;
	}
}
...
// Right Hand information release version
void NuitrackGLSample::onHandUpdate(HandTrackerData::Ptr handData)
{
	...
	if (!rightHand)
	{
		...
		//	"Rx = " << rightHand->xReal << ", "
		//	"Ry = " << rightHand->yReal << ", "
		//	"Rz = " << rightHand->zReal << std::endl;

		RHandPos[0] = rightHand->xReal;
		RHandPos[1] = rightHand->yReal;
		RHandPos[2] = rightHand->zReal;
	}
	...
	if (!leftHand)
	{
		...
		//	"Lx = " << leftHand->xReal << ", "
		//	"Ly = " << leftHand->yReal << ", "
		//	"Lz = " << leftHand->zReal << std::endl;
		LHandPos[0] = leftHand->xReal;
		LHandPos[1] = leftHand->yReal;
		LHandPos[2] = leftHand->zReal;
	}
}


////previous code___ nuitrack_gl_sample
//void NuitrackGLSample::onHandUpdate(HandTrackerData::Ptr handData)
//{
